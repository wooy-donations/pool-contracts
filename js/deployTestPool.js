...
async function deployTestPool({
  ...
  if (poolType == 'stake') {
    ...
    const stakePoolConfig = { token: tokenResult.address, maxExitFeeMantissa, beneficiaryAddress };
    ...
    let tx = await poolBuilder.createCompoundMultipleWinners(
    ...
    let tx = await poolBuilder.createYieldSourceMultipleWinners(
