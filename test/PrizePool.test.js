...
describe('PrizePool', function() {
  ...
  describe('initialize()', () => {
    it('should fire the events', async () => {
      let tx = prizePool.initializeAll(reserve.address, [ticket.address], poolMaxExitFee, yieldSourceStub.address);

      await expect(tx)
        .to.emit(prizePool, 'Initialized')
        .withArgs(reserve.address, poolMaxExitFee);
  ...
  describe('with a mocked prize pool', () => {
    beforeEach(async () => {
      await prizePool.initializeAll(reserveRegistry.address, [ticket.address], poolMaxExitFee, yieldSourceStub.address);
    ...
    describe('beforeTokenTransfer()', () => {
      it('should not allow uncontrolled tokens to call', async () => {
        await expect(prizePool.beforeTokenTransfer(wallet.address, wallet2.address, toWei('1'))).to.be.revertedWith(
      ...
      it('should accrue credit to the sender and receiver', async () => {
        ...
        await prizeStrategy.mock.beforeTokenTransfer
          .withArgs(wallet.address, wallet2.address, toWei('50'), ticket.address)
          .returns();
    ...
    describe('initialize()', () => {
      ...
      it('should reject invalid params', async () => {
        const _initArgs = [reserveRegistry.address, [ticket.address], poolMaxExitFee, yieldSourceStub.address];
        let initArgs;
    ...
    describe('setBeneficiary()', () => {
      it('should revert with setted address zero', async () => {
        await expect(prizePool.setBeneficiary(AddressZero)).to.be.revertedWith(
          'Beneficiary address should not be address zero'
        );
      });
    ...
    describe('depositTo()', () => {
      it('should revert when deposit exceeds liquidity cap', async () => {
        ...
        await expect(prizePool.depositTo(wallet2.address, amount, ticket.address, AddressZero)).to.be.revertedWith(
    ...
    describe('setPrizeStrategy()', () => {
      ...
      it('should not allow anyone else to change the prize strategy', async () => {
        await expect(prizePool.connect(wallet2).setPrizeStrategy(wallet2.address)).to.be.revertedWith(
    ...
    describe('compLikeDelegate()', () => {
      ...
      it('should only allow the owner to delegate', async () => {
        await expect(prizePool.connect(wallet2).compLikeDelegate(compLike.address, wallet2.address)).to.be.revertedWith(
          'Ownable: caller is not the owner'
        );
      });
  ...
  describe('awardExternalERC20()', () => {
    beforeEach(async () => {
      await prizePool.initializeAll(prizeStrategy.address, [ticket.address], poolMaxExitFee, yieldSourceStub.address);
      await prizePool.setPrizeStrategy(prizeStrategy.address);
      await prizePool.setCreditPlanOf(ticket.address, toWei('0.01'), toWei('0.1'));
    });
    ...
    it('should exit early when amount = 0', async () => {
      ...
      await expect(
        prizeStrategy.call(prizePool, 'awardExternalERC20', wallet.address, erc20token.address, 0)
    ...
    it('should only allow the prizeStrategy to award external ERC20s', async () => {
      ...
      await expect(prizePool2.awardExternalERC20(wallet.address, wallet2.address, toWei('10'))).to.be.revertedWith(
  ...
  describe('transferExternalERC20()', () => {
    ...
    it('should exit early when amount = 0', async () => {
      await yieldSourceStub.mock.canAwardExternal.withArgs(erc20token.address).returns(true);
      await expect(
        prizeStrategy.call(prizePool, 'transferExternalERC20', wallet.address, erc20token.address, 0)
    ...
    it('should only allow the prizeStrategy to award external ERC20s', async () => {
      await yieldSourceStub.mock.canAwardExternal.withArgs(erc20token.address).returns(true);
      let prizePool2 = prizePool.connect(wallet2);
      await expect(prizePool2.transferExternalERC20(wallet.address, wallet2.address, toWei('10'))).to.be.revertedWith(
        'PrizePool/only-prizeStrategy'
      );
    });
    ...
    it('should allow arbitrary tokens to be transferred', async () => {
      ...
      await erc20token.mock.transfer.withArgs(wallet.address, toWei('10')).returns(true);
      await expect(
        prizeStrategy.call(prizePool, 'transferExternalERC20', wallet.address, erc20token.address, toWei('10'))
      )
  ...
  describe('awardExternalERC721()', () => {
    ...
    it('should exit early when tokenIds list is empty', async () => {
      ...
      await expect(
        prizeStrategy.call(prizePool, 'awardExternalERC721', wallet.address, erc721token.address, [])
    ...
    it('should only allow the prizeStrategy to award external ERC721s', async () => {
      ...
      let prizePool2 = prizePool.connect(wallet2);
      await expect(
        prizePool2.awardExternalERC721(wallet.address, erc721token.address, [NFT_TOKEN_ID])
    ...
    it('should allow arbitrary tokens to be transferred', async () => {
      ...
      await expect(
        prizeStrategy.call(prizePool, 'awardExternalERC721', wallet.address, erc721token.address, [NFT_TOKEN_ID])
      )
