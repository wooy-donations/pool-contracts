...
function PoolEnv() {
  ...
  this.createPool = async function({
    ...
    for (var i = 0; i < externalERC20Awards.length; i++) {
      this.externalERC20Awards[externalERC20Awards[i]] = await ERC20Mintable.deploy(
        `External ERC20 Token ${i + 1}`,
        `ETKN${i + 1}`
      );
  ...
  this.withdrawInstantly = async function({ user, tickets }) {
    ...
    let prizePool = await this.prizePool(wallet);
    await prizePool['withdrawInstantlyFrom(address,uint256,address,uint256)'](
      wallet.address,
      withdrawalAmount,
      ticket.address,
      toWei('1000')
